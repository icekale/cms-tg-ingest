...
**0.5.2** 使用一份 SQLite 任务库和唯一 TaskRunner 写入者；观察者只入队命令。当前镜像 `icekale/cms-tg-ingest:0.5.34`。
...
services:
  cms-tg-ingest:
    image: icekale/cms-tg-ingest:0.5.34
...
- `/订阅 https://re0.me/tv/<slug>`：创建 HDHive 剧集订阅。
...
订阅支持智能判断：集数过滤示例为 `S01E01-S01E10,S02`，默认跳过 `S00` 特殊集；多季资源按季集编号识别，Emby 已有集数会跳过。程序根据 TMDB 完结状态和季集数量标记已完结订阅，第一次完结发一条 Telegram，之后不再每日检查；无法解析季集、费用未知或超过自动阈值的资源不自动解锁，完结订阅可手动恢复。
