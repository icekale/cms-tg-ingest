...
_JUNK_VIDEO = re.compile(r"(?i)(请访问|更多精彩|更多影视|更多资源|尽在www\.|DDHDTV)")
...
def is_junk_video_name(name: str) -> bool:
    return is_video_name(name) and bool(_JUNK_VIDEO.search(str(name or "")))
...
def snapshot_files(roots: list[dict[str, Any]], list_files: ListFiles) -> list[dict[str, str]]:
    ...
    def add(file_id: str, name: str) -> None:
        ...
        if not file_id or not is_video_name(name) or is_junk_video_name(name) or file_id in seen:
            return
        seen.add(file_id)
