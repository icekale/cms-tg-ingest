...
class BridgeSelfShareTaskWorkflowTests(unittest.TestCase):
    ...
    def test_multi_target_cleaned_review_uses_each_target_share_credentials(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for target in task.metadata["organized_targets"]:
                target["share"]["review"] = {}
                target["strm"].update({"status": "ready", "source_path": str(Path(tmp) / "share-strm" / target["folder"]["file_name"]), "move_status": "moved", "emby_status": "confirmed", "dest_path": str(Path(tmp) / "library" / target["folder"]["file_name"])})
            seen = []
            def review(_task, row):
                seen.append((row["own_share_code"], row["own_share_receive_code"], row["own_share_file_id"]))
                return "passed", {"share_review_status": "passed"}, "", 0
            workflow._advance_share_review = review
            result = workflow.run_stage(task)
            self.assertEqual(seen, [("own-a", "ownpwd", "dest-a"), ("own-b", "ownpwd", "dest-b")])
            self.assertIn(result.outcome, {StageOutcome.COMPLETE, StageOutcome.NEEDS_ACTION})

    def test_multi_target_cleaned_without_cleanup_client_still_enforces_all_gates(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow, task, _row = self._multi_target_stage_fixture(tmp, stage=TaskStage.CLEANED, cleanup=None)
            targets = task.metadata["organized_targets"]
            for target in targets:
                target["strm"].update({"status": "ready", "move_status": "moved", "emby_status": "confirmed"})
            targets[1]["share"]["status"] = "pending"
            result = workflow.run_stage(task)
            self.assertEqual(result.outcome, StageOutcome.NEEDS_ACTION)

            targets[1]["share"]["status"] = "succeeded"
            task.metadata["intake_identity"]["files"] = []
            result = workflow.run_stage(task)
            self.assertEqual(result.outcome, StageOutcome.NEEDS_ACTION)

    def test_multi_target_cleanup_rejects_intake_file_union_mismatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            cleanup = FakeCleanupClient()
            workflow, task, _row = self._multi_target_stage_fixture(tmp, stage=TaskStage.CLEANED, cleanup=cleanup)
            for target in task.metadata["organized_targets"]:
                target["strm"].update({"status": "ready", "move_status": "moved", "emby_status": "confirmed", "dest_path": str(Path(tmp) / "library" / target["folder"]["file_name"])})
            task.metadata["intake_identity"]["files"] = [{"id": "episode-a"}]
            result = workflow.run_stage(task)
            self.assertEqual(result.outcome, StageOutcome.NEEDS_ACTION)
            self.assertEqual(cleanup.deleted, [])

    def test_multi_target_strm_ready_skips_completed_target_work(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow, task, _row = self._multi_target_stage_fixture(tmp)
            for target in task.metadata["organized_targets"]:
                target["strm"].update({"status": "ready", "source_path": str(Path(tmp) / "share-strm" / target["folder"]["file_name"]), "playback_validated": True})
            with patch("app.workflows.self_share.find_self_share_strm_source_dir", side_effect=AssertionError("rediscovered completed target")), patch("app.workflows.self_share.restore_canonical_strm_paths", side_effect=AssertionError("restored completed target")):
                result = workflow.run_stage(task)
            self.assertEqual(result.outcome, StageOutcome.COMPLETE)
            self.assertEqual(workflow.cms.playback_calls, [])

    def test_multi_target_strm_ready_rechecks_source_ready_target(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow, task, _row = self._multi_target_stage_fixture(tmp)
            target = task.metadata["organized_targets"][0]
            source = Path(tmp) / "share-strm" / target["folder"]["file_name"]
            target["strm"].update({"status": "source_ready", "source_path": str(source)})
            result = workflow.run_stage(task)
            self.assertEqual(result.outcome, StageOutcome.COMPLETE)
            self.assertEqual(result.metadata["organized_targets"][0]["strm"]["status"], "ready")
            self.assertTrue(result.metadata["organized_targets"][0]["strm"]["playback_validated"])

    def test_multi_target_emby_confirmed_skips_duplicate_notifications(self):
        with tempfile.TemporaryDirectory() as tmp:
            emby = FakeEmby()
            workflow, task, _row = self._multi_target_stage_fixture(tmp, stage=TaskStage.EMBY_CONFIRMED, emby=emby)
            for target in task.metadata["organized_targets"]:
                target["strm"].update({"status": "ready", "move_status": "moved", "emby_status": "confirmed", "dest_path": str(Path(tmp) / "library" / target["folder"]["file_name"])})
            with patch("app.workflows.self_share.send_emby_confirmed", side_effect=AssertionError("reconfirmed target")):
                result = workflow.run_stage(task)
            self.assertEqual(result.outcome, StageOutcome.COMPLETE)

    def test_multi_target_moved_preflights_later_plan_error_before_any_move(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow, task, _row = self._multi_target_stage_fixture(tmp, stage=TaskStage.MOVED)
            original = bridge.plan_strm_move
            def plan(source, category, config, **kwargs):
                if Path(source).name.startswith("B-"):
                    raise ValueError("deterministic target B plan error")
                return original(source, category, config, **kwargs)
            with patch("app.workflows.self_share.plan_strm_move", side_effect=plan):
                result = workflow.run_stage(task)
            self.assertEqual(result.outcome, StageOutcome.NEEDS_ACTION)
    ...
    def test_organizing_stage_rejects_duplicate_folder_multi_target_identity(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.outcome, StageOutcome.NEEDS_ACTION)
            self.assertIn("多目录状态损坏", result.message)
    ...
    def test_multi_target_owner_without_target_tmdb_needs_action(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            row = self._row()
            targets = self._multi_targets()
            for target in targets:
                target["folder"]["file_name"] = target["folder"]["file_name"].split("-[tmdb=")[0]
                target["recognition"] = {}
            ...
            self.tasks.record_event(
                ...
                TaskStatus.PENDING,
                "owner",
                metadata_patch={"own_share_file_id": "dest-a", "recognition": {"tmdb_id": "259231"}},
            )
    ...
    def test_multi_target_schema_requires_folder_share_and_strm_fields(self):
        malformed = (
            ("folder", "file_name", ""),
            ("folder", "parent_id", ""),
            ("share", "status", ""),
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for section, field, value in malformed:
                for stage in (TaskStage.RECOGNIZING, TaskStage.SHARE_ALIAS_PREPARED, TaskStage.OWN_SHARE_CREATED):
                    workflow = self._workflow(tmp)
                    row = self._row()
                    targets = self._multi_targets()
                    targets[0][section][field] = value
                    task = self._multi_target_task(workflow, stage, targets=targets, row=row)
    ...
    def test_multi_target_rejects_duplicate_file_ownership_across_targets(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            row = self._row()
            targets = self._multi_targets()
            targets[1]["file_ids"] = [targets[0]["file_ids"][0]]
            task = self._multi_target_task(workflow, TaskStage.SHARE_ALIAS_PREPARED, targets=targets, row=row)

            result = workflow.run_stage(task)

            self.assertEqual(result.outcome, StageOutcome.NEEDS_ACTION)
            self.assertIn("文件归属重复", result.message)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for field in malformed_fields:
                for stage in (TaskStage.RECOGNIZING, TaskStage.SHARE_ALIAS_PREPARED, TaskStage.OWN_SHARE_CREATED):
                    ...
                    row = self._row()
                    targets = self._multi_targets()
                    targets[0][field] = "bad" if field != "file_ids" else "abc"
                    task = self._claim_task(
                        "abc", "1234", stage,
                        {"submission_id": row["id"], "multi_target_version": 1, "organized_targets": targets},
                        row["id"],
                    )
    ...
    def test_multi_target_share_file_id_missing_needs_action_before_alias_or_share(self):
        with tempfile.TemporaryDirectory() as tmp:
            for stage in (TaskStage.SHARE_ALIAS_PREPARED, TaskStage.OWN_SHARE_CREATED):
                workflow = self._workflow(tmp)
                row = self._row()
                targets = self._multi_targets()
                targets[0]["share"].pop("file_id")
                task = self._multi_target_task(workflow, stage, targets=targets, row=row)
    ...
    def test_multi_target_later_malformed_prepared_operation_prevents_all_share_creation(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            row = self._row()
            task = self._multi_target_task(workflow, TaskStage.OWN_SHARE_CREATED, row=row)
            ...
            self.tasks.prepare_operation(
                ...
                operation_key,
                "create_share",
                {"file_id": "wrong-file", "target_id": "dest-b", "share_title": "B", "receive_code": "1212"},
            )
    ...
    def test_multi_target_succeeded_code_must_match_trusted_operation_result(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            row = self._row()
            targets = self._multi_targets()
            for target in targets:
                target["share"].update({"status": "succeeded", "code": "state-code"})
            task = self._multi_target_task(workflow, TaskStage.OWN_SHARE_CREATED, targets=targets, row=row)
            for target in targets:
                target_id = target["target_id"]
                key = f"{operation_scope(task)}:create_share:{target_id}"
                self.tasks.prepare_operation(task.id, key, "create_share", {"file_id": target_id, "target_id": target_id})
                self.tasks.start_operation(task.id, key)
                self.tasks.complete_operation(task.id, key, {"share_code": "trusted-code"})
    ...
    def test_multi_target_missing_or_invalid_intake_identity_needs_action_before_share(self):
        with tempfile.TemporaryDirectory() as tmp:
            for identity in (None, {}, {"files": []}, {"files": ["bad"]}):
                workflow = self._workflow(tmp)
                row = self._row()
                task = self._multi_target_task(workflow, TaskStage.OWN_SHARE_CREATED, row=row)
                task.metadata["intake_identity"] = identity
    ...
    def test_multi_target_bogus_share_status_needs_action_before_share(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            row = self._row()
            targets = self._multi_targets()
            targets[1]["share"]["status"] = "bogus"
            task = self._multi_target_task(workflow, TaskStage.OWN_SHARE_CREATED, targets=targets, row=row)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(RuntimeError):
                workflow._journaled_create_share(
                    ...
                    target_id="dest-a",
                    recovery_metadata={"file_id": "dest-b"},
                )
    ...
    def test_multi_target_prepared_share_identity_mismatch_does_not_start_operation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(self.tasks.find_operation(task.id, operation_key).status, "prepared")
            self.assertEqual(workflow.p115.created_shares, [])
    ...
    def test_multi_target_create_share_ignores_previous_generation_operations(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            row = self._row()
            task = self._claim_task(
                ...
                TaskStage.OWN_SHARE_CREATED,
                {
                    ...
                    "update_requested_run": 2,
                    "multi_target_version": 1,
                    "organized_targets": self._multi_targets(),
                    "intake_identity": {
                        "root_ids": ["received-root"],
                        "files": [{"id": "episode-a"}, {"id": "episode-b"}],
                    },
                },
                row["id"],
            )
            old_generation_key = "g0:u2:create_share:dest-a"
            old_update_key = "g1:u1:create_share:dest-b"
            for old_key, target_id in ((old_generation_key, "dest-a"), (old_update_key, "dest-b")):
                self.tasks.prepare_operation(task.id, old_key, "create_share", {"target_id": target_id, "file_id": target_id})
                self.tasks.start_operation(task.id, old_key)
                self.tasks.complete_operation(task.id, old_key, {"target_id": target_id, "file_id": target_id, "share_code": "old-share"})
            ...
            self.assertEqual(self.p115.created_shares, ["dest-a", "dest-b"])
            self.assertEqual(
                {operation.operation_key for operation in self.tasks.list_operations(task.id) if operation.operation_type == "create_share"},
                {old_generation_key, old_update_key, "g1:u2:create_share:dest-a", "g1:u2:create_share:dest-b"},
            )

    def test_multi_target_create_share_rejects_succeeded_result_target_identity_before_posting(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            targets = self._multi_targets()
            for target, code in ((targets[0], "share-a"), (targets[1], "share-b")):
                target["share"].update({"status": "succeeded", "code": code, "receive_code": "pwd", "validation_status": "valid"})
            task = self._multi_target_task(workflow, TaskStage.OWN_SHARE_CREATED, targets=targets)
            for target in targets:
                target_id = target["target_id"]
                key = f"{operation_scope(task)}:create_share:{target_id}"
                self.tasks.prepare_operation(task.id, key, "create_share", {"target_id": target_id, "file_id": target_id})
                self.tasks.start_operation(task.id, key)
                result = {"share_code": target["share"]["code"], "target_id": target_id}
                if target_id == "dest-a":
                    result["target_id"] = "wrong-target"
                self.tasks.complete_operation(task.id, key, result)
    ...
    def test_single_target_cms_sync_uses_default_receive_code_when_history_is_blank(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            row = self._row()
            row = self.submissions.update_self_share(
            ...
            self.assertEqual(result.outcome, StageOutcome.COMPLETE)
            self.assertEqual(self.cms.share_sync_calls, [("owncode", "1212", "0", "/media/share")])
    ...
    def test_multi_target_share_validation_inspects_each_target_and_preserves_first_result(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.p115.share_statuses = [
                ...
                {"available": False, "share_state": "2", "have_vio_file": True},
            ]
            targets = self._multi_targets()
            ...
            self.assertEqual(result.outcome, StageOutcome.NEEDS_ACTION)
            self.assertEqual(self.p115.inspect_calls, [("share-a", "pwd-a"), ("share-b", "pwd-b")])
            saved = result.metadata["organized_targets"]
            self.assertEqual(saved[0]["share"]["validation_status"], "valid")
            self.assertEqual(saved[0]["share"]["validation_error"], "")
            self.assertEqual(saved[1]["share"]["validation_status"], "invalid")
    ...
    def test_multi_target_share_validation_completes_when_all_targets_are_valid(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.p115.share_statuses = [
                {"available": True, "share_state": "0", "have_vio_file": False},
                {"available": True, "share_state": "1", "have_vio_file": False},
            ]
            targets = self._multi_targets()
            targets[0]["share"].update({"status": "succeeded", "code": "share-a", "receive_code": "pwd-a"})
            targets[1]["share"].update({"status": "succeeded", "code": "share-b", "receive_code": "pwd-b"})
            ...
            self.assertEqual(result.outcome, StageOutcome.COMPLETE)
            self.assertEqual([target["share"]["validation_status"] for target in result.metadata["organized_targets"]], ["valid", "valid"])

    def test_multi_target_cms_sync_is_journaled_per_target_and_idempotent(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            targets = self._multi_targets()
            for target, code, pwd in ((targets[0], "share-a", "pwd-a"), (targets[1], "share-b", "pwd-b")):
            ...
            self.assertEqual(sorted(op.operation_key for op in operations), [
                f"{operation_scope(task)}:cms_share_sync:dest-a:share-a",
                f"{operation_scope(task)}:cms_share_sync:dest-b:share-b",
            ])

    def test_multi_target_cms_sync_accepts_historical_result_without_target_identity_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            targets = self._multi_targets()
            for target, code, pwd in ((targets[0], "share-a", "pwd-a"), (targets[1], "share-b", "pwd-b")):
            ...
            for target in targets:
                target_id = target["target_id"]
                code = target["share"]["code"]
                create_key = f"{operation_scope(task)}:create_share:{target_id}"
                self.tasks.prepare_operation(task.id, create_key, "create_share", {"target_id": target_id, "file_id": target_id})
                self.tasks.start_operation(task.id, create_key)
                self.tasks.complete_operation(task.id, create_key, {"target_id": target_id, "file_id": target_id, "share_code": code})
            sync_key = f"{operation_scope(task)}:cms_share_sync:dest-a:share-a"
            self.tasks.prepare_operation(
                task.id,
                sync_key,
    ...
    def test_multi_target_cms_sync_prevalidates_all_create_operations_before_posting(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for target, code, pwd in ((targets[0], "share-a", "pwd-a"), (targets[1], "share-b", "pwd-b")):
                target["share"].update({"status": "succeeded", "code": code, "receive_code": pwd, "validation_status": "valid"})
            task = self._multi_target_task(workflow, TaskStage.SHARE_SYNC_SUBMITTED, targets=targets)
            key = f"{operation_scope(task)}:create_share:dest-a"
            self.tasks.prepare_operation(task.id, key, "create_share", {"target_id": "dest-a", "file_id": "dest-a"})
            self.tasks.start_operation(task.id, key)
            self.tasks.complete_operation(task.id, key, {"target_id": "dest-a", "file_id": "dest-a", "share_code": "share-a"})
    ...
    def test_multi_target_cms_sync_stops_safely_after_later_uncertain_target(self):
        with tempfile.TemporaryDirectory() as tmp:
            workflow = self._workflow(tmp)
            workflow.cms.add_share115_sync_task = Mock(side_effect=[None, RuntimeError("cms unavailable")])
            targets = self._multi_targets()
            for target, code, pwd in ((targets[0], "share-a", "pwd-a"), (targets[1], "share-b", "pwd-b")):
                target["share"].update({"status": "succeeded", "code": code, "receive_code": pwd, "validation_status": "valid"})
            task = self._multi_target_task(workflow, TaskStage.SHARE_SYNC_SUBMITTED, targets=targets)
            ...
            self.assertEqual(workflow.cms.add_share115_sync_task.call_count, 2)
