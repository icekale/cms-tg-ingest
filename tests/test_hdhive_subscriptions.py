...
class HdhiveSubscriptionServiceTests(unittest.TestCase):
    ...
    def test_llm_medium_confidence_is_pending_without_unlock(self):
        # Multi-season TMDB so regex cannot apply default_season to "更新至07集".
        tmdb = FakeTmdbResolver(
            {
                "ok": True,
                "seasons": [
    ...
    def test_llm_is_not_called_without_clue_or_when_regex_parses(self):
        ...
        directory, store, subscription, proxy, service, _intake = self.make_service(
            [
                ...
                # Above auto-unlock threshold so regex-parsed items stay pending and unlock is not called.
                resource("parsed", episode_key="", title="Show", remark="S01E02", points=21),
            ],
            tmdb_resolver=tmdb,
            episode_parser=parser,
        )
    ...
    def test_llm_pending_range_does_not_cover_regex_episode(self):
        tmdb = FakeTmdbResolver(
            {
                "ok": True,
                "seasons": [
        ...
        directory, store, subscription, proxy, service, intake_calls = self.make_service(
            [
                resource("llm1080", episode_key="", title="剧", remark="更新至07集", resolution="1080P", points=0),
                resource("regex2160", episode_key="", title="Show", remark="S02E07", resolution="2160P", points=8),
            ],
            [HdhiveUnlockItem("regex2160", True, "https://115cdn.com/s/regex2160?password=abcd", "", "", False)],
            tmdb_resolver=tmdb,
        ...
        self.assertEqual(result.enqueued, 1)
        self.assertEqual(items["regex2160"].status, "enqueued")
        self.assertNotEqual(items["regex2160"].status, "filtered")
        self.assertEqual(items["regex2160"].normalized_episode_key, "S02E07")
        self.assertEqual(items["llm1080"].status, "pending_confirmation")
        self.assertIn("LLM 识别待确认：S02E01-S02E07", items["llm1080"].last_error)
        self.assertEqual(proxy.unlock_calls, [["regex2160"]])
        self.assertEqual(intake_calls, [(["https://115cdn.com/s/regex2160?password=abcd"], "464100862")])

    def test_regex_superset_covers_llm_pending_subset(self):
        tmdb = FakeTmdbResolver(
            {
                "ok": True,
                "seasons": [
        ...
        directory, store, subscription, proxy, service, intake_calls = self.make_service(
            [
                resource("regex2160", episode_key="", title="Show", remark="S02E01-S02E08", resolution="2160P", points=8),
                resource("llm1080", episode_key="", title="剧", remark="更新至07集", resolution="1080P", points=0),
            ],
            [HdhiveUnlockItem("regex2160", True, "https://115cdn.com/s/regex2160?password=abcd", "", "", False)],
            tmdb_resolver=tmdb,
        ...
        self.assertEqual(result.enqueued, 1)
        self.assertEqual(items["regex2160"].status, "enqueued")
        self.assertEqual(items["llm1080"].status, "filtered")
        self.assertEqual(items["llm1080"].skip_reason, "集数已被覆盖更完整的其他资源包含")
        self.assertNotEqual(items["llm1080"].status, "pending_confirmation")
        self.assertEqual(proxy.unlock_calls, [["regex2160"]])
        self.assertEqual(intake_calls, [(["https://115cdn.com/s/regex2160?password=abcd"], "464100862")])
