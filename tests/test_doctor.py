...
class CmsStrmGuardCheckTests(unittest.TestCase):
    ...
    def test_guard_skipped_for_direct_workflow(self):
        ...
        os_guard = next(i for i in item if i.name == "cms_os_strm_guard")
        ...
        self.assertIn("only relevant for self_share_sync", os_guard.message)
    ...
    def test_guard_ok_when_marker_found_in_logs(self):
        ...
        self.assertTrue(os_guard.ok)
        self.assertIn("已安装", os_guard.message)
    ...
    def test_guard_fails_when_marker_missing(self):
        ...
        self.assertFalse(os_guard.ok)
        self.assertIn("未找到", os_guard.message)
        self.assertIn("静默失效", os_guard.message)
        self.assertEqual(reader.calls, [("cloud-media-sync", 100000)])
    ...
    def test_guard_warns_when_docker_unavailable(self):
        ...
        self.assertTrue(os_guard.ok)
        self.assertIn("无法读取", os_guard.message)
