...
class OpenAIClassifier:
    ...
    def __init__(self, config: Config, http: HttpJson | None = None):
        ...
        self.http = http or HttpJson(min(int(getattr(config, "http_timeout", 60) or 60), 90))
        ...
        try:
            ...
            def on_update(text: str) -> None:
                if not placeholder_id or not text:
                    return
                now = time.time()
                if now - state["last_edit"] < 2.5:
                    return
                state["last_edit"] = now
                try:
                    telegram.edit_message_text(chat_id, placeholder_id, ("🤖 " + text)[:3800] + " …")
                except Exception:
                    LOG.debug("assistant streaming edit failed", exc_info=True)

            # 首个 token 前保持“正在输入”指示
            ...
            try:
                ...
                typing_stop.set()
            reply = result["reply"]
            chunks = _chunk_assistant_text(f"{header}{reply}")
            if placeholder_id:
                try:
                    telegram.edit_message_text(chat_id, placeholder_id, chunks[0])
                    sent_first = True
                except Exception:
            ...
            if placeholder_id:
                try:
                    telegram.edit_message_text(chat_id, placeholder_id, message)
                    return
                except Exception:
