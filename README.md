...
3. 使用 Docker Hub 完整 Compose 配置，或在 Unraid Compose Manager 中创建 `cms-tg-ingest` 服务，并将镜像设置为 `icekale/cms-tg-ingest:0.2.46`。
4. 拉取固定版本并启动：
...
```sh
docker compose down
docker pull icekale/cms-tg-ingest:0.2.45
# 将 compose 的 image 改为 0.2.45
docker compose up -d
```
