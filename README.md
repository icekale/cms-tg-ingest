...
- 可选安全清理 115 转存源：只会在自己的永久分享已存在、STRM 已生成并移动到媒体库、且 Emby 确认入库后删除，不会取消自己的 115 永久分享。
...
9. 如果启用清理，外挂只会在自己的永久分享已存在、STRM 已生成并移动到媒体库、且 Emby 确认入库后删除 115 转存源文件或源文件夹，不取消你自己的永久分享。
...
- `SELF_SHARE_CLEANUP_AFTER_EMBY=true` 只会在自己的永久分享已存在后执行，并且只会在 STRM 已移动且 Emby 确认入库后删除 115 转存源，不会取消你自己的永久分享；变量名保留历史兼容。
