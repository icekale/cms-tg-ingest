...
```sh
docker pull icekale/cms-tg-ingest:0.2.14
docker compose up -d
```
...
- Vue 任务详情页和旧任务详情页都提供重试、查 Emby、恢复 STRM、从头重跑按钮，并显示事件时间线、阶段耗时和 115 调用统计。
- /health 会显示 TaskStore 本地队列健康、worker 心跳和 115 风控冷却。
- /quality 会先执行 TaskStore 本地轻量巡检。
- Vue 和旧版 `/quality` 页面都支持本地巡检、自动巡检设置和修复入队；只读取本地 TaskStore 和 STRM 文件，不扫描 115。
