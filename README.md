...
- **两种 STRM 模式**：`shared` 使用自己的 115 分享链接生成 STRM，并保留分享；`direct` 使用 CMS 普通同步生成的直链 STRM，不创建自有分享，也不清理源文件。
...
- **Vue 管理壳**：访问 `/app/` 使用轻量 Vue/Naive UI 页面；旧 Python 页面和 POST 路由继续保留。
...
只读 API 位于 `/api/v1/overview`、`/api/v1/tasks`、`/api/v1/health`、`/api/v1/quality` 和 `/api/v1/hdhive`。它们沿用 Web Token 认证，不返回 Cookie、访问令牌或 115 分享密码；模式设置使用对应的 POST API。
